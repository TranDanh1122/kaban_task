...
export default function SettingDialog() {
    ...
    React.useEffect(() => {
        ...
        if (setting.font == nunito.className) {
            document.documentElement.classList.remove(jakarta.className);
            document.documentElement.classList.add(nunito.className);
        }
        if (setting.font == jakarta.className) {
            document.documentElement.classList.remove(nunito.className);
            document.documentElement.classList.add(jakarta.className);
        }
    ...
    return <Dialog onOpenChange={(open) => dispatch({ type: "TOOGLE", payload: { name: "SettingDialog", state: open } })} open={isOpen("SettingDialog")}>
        ...
        <DialogContent>
            ...
            <DialogFooter className="flex items-center sm:justify-between">
                ...
                <Button onClick={() => { }} className="rounded-2xl bg-primary-300 hover:bg-primary-300 ">
                    Apply
                </Button>
            </DialogFooter>
        </DialogContent>
